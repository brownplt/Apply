...
if len(sys.argv) > 1:
	if sys.argv[1] == 'initdb':
		...
		importer.importTranscripts(sys.argv[2])
		sys.exit(0)
	elif sys.argv[1] == 'importReferences':
	        if not sys.argv[2]:
			print "Please specify the directory the references are held in."
	        importer.importReferences(sys.argv[2])
		sys.exit(0)
	elif sys.argv[1] == 'importFullApps':
	        if not sys.argv[2]:
			print "Please specify the directory the references are held in."
	        importer.importFullApps(sys.argv[2])
		sys.exit(0)
	elif sys.argv[1] == 'populateFromCSV':
		if not sys.argv[2]:
			print "Please specify a filename to use for input."
		importer.populateFromCSV(sys.argv[2])
		sys.exit(0)
